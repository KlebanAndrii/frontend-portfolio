    ...
    <header>
      ...
      >
        <div className="container mx-auto flex items-center justify-between px-6 py-4">
          ...
          <div className="hidden items-center gap-8 md:flex">
            {navLinks.map((link) => (
              <div key={link.name} className="transition-opacity duration-500">
                <Link href={link.href} className="group relative">
                  <span className="font-semibold text-gray-700 transition hover:text-sky-600 dark:text-gray-200 dark:hover:text-sky-400">
                    {link.name}
                  </span>
        ...
        {isOpen && (
          <div
            id="mobile-menu"
            className="mx-2 mb-4 flex flex-col items-center gap-6 rounded-2xl border border-neutral-300 bg-white/30 p-6 shadow-xl backdrop-blur-md transition-all duration-300 md:hidden dark:border-neutral-700 dark:bg-neutral-800/30"
          >
            {navLinks.map((link) => (
